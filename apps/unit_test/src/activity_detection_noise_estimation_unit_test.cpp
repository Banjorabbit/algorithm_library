...
TEST(ActivityDetectionNoiseEstimation, Interface) { EXPECT_TRUE(InterfaceTests::algorithmInterfaceTest<ActivityDetectionNoiseEstimation>()); }
...
TEST(ActivityDetectionFusedNoiseEstimation, ActivityDetect)
{
    ...
    for (auto n = 0; n < 51; n++)
    {
        ...
        if (n % 10 == 0) { EXPECT_TRUE(activity); }
        else { EXPECT_FALSE(activity); }
    }
}
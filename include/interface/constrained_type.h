...
public:
	...
	ConstrainedType(const T& valueNew, const T& vMinNew, const T& vMaxNew) : vMin(vMinNew), vMax(vMaxNew) { *this = valueNew; }; // constructor, uses assignment operator to check value
	ConstrainedType(const T& valueNew, const T& vMinNew, const T& vMaxNew, const Scalings& scalingNew, const float& scaleFactorNew, const Units& unitNew) : vMin(vMinNew), vMax(vMaxNew), scaling(scalingNew), scaleFactor(scaleFactorNew), unit(unitNew) { *this = valueNew; }; // constructor, uses assignment operator to check value
	...
	operator T() const { return value; } // cast to T type (used when assigning to other variable / expression). Some optimization error results in this failing if return type is changed to const reference

	void setMax(const T& vMaxNew) { vMax = vMaxNew; value = (std::min)(vMax, value); }
	void setMin(const T& vMinNew) { vMin = vMinNew; value = (std::max)(vMin, value); }

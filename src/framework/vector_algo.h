...
public:
    ...
    auto getCoefficientsTree() const
    {
        ...
        for (size_t i = 0; i < vec.size(); i++) { cVec[i] = vec[i].getCoefficientsTree(); }
        return cVec;
    }
    ...
    auto getParametersTree() const
    {
        ...
        for (size_t i = 0; i < vec.size(); i++) { pVec[i] = vec[i].getParametersTree(); }
        return pVec;
    }
    ...
    auto getSetupTree() const
    {
        ...
        for (size_t i = 0; i < vec.size(); i++) { sVec[i] = vec[i].getSetupTree(); }
        return sVec;
    }

    // set
    void setCoefficients(const Coefficients& c) { for (size_t i = 0;i < size();i++) { vec[i].setCoefficients(c); } }

    template<typename Tcoefficients>
    void setCoefficients(const Tcoefficients& c) 
    { 
        if (c.size() != size()) { resize(c.size); } 
        for (size_t i = 0; i < size(); i++) { vec[i].setCoefficients(c[i]); } 
    }

    void setParameters(const Parameters& p) { for (size_t i = 0;i < size();i++) { vec[i].setParameters(p); } }

    template<typename Tparameters>
    void setParameters(const Tparameters& p) 
    { 
        if (p.size() != size()) { resize(p.size()); }
        for (size_t i = 0; i < size(); i++) { vec[i].setParameters(p[i]); } 
    }

    void setSetup(const Setup& s) { for (size_t i = 0;i < size();i++) { vec[i].setSetup(s); } }

    template<typename Tsetup>
    void setSetup(const Tsetup& s) 
    { 
        if (s.size() != size()) { resize(s.size()); }
        for (size_t i = 0; i < size(); i++) { vec[i].setSetup(s[i]); } 
    }
    ...
    void setParametersTree(const Tparameters& p)
    {
        ...
        for (size_t i = 0; i < size(); i++) { vec[i].setParametersTree(p[i]); }
    }
    ...
    { 
        if (s.size() != size()) { resize(s.size()); }
        for (size_t i = 0; i < size(); i++) { vec[i].setSetupTree(s[i]); } 
    }

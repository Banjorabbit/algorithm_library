...
public:
    ...
    void processOn(Input noisyPower, Output activityFlag)
	{
        ...
        activityFlag = (noisyPower.colwise().mean().transpose() > (noisePower.colwise().mean().transpose() * 1.5 * P.activityThreshold)).any(); // mean power of each channel is compared. ActivityFlag is true if any channel is above threshold

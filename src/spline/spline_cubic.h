        ...
        for (auto channel = 0; channel < x.xGiven.cols(); channel++)
        {
            ...
            for (auto i = 0; i < C.nGiven - 1; i++)
            {
                ...
                for (auto idx = 0; idx < idxDiff; idx++)
                {
                    ...
                    y(index(i) + idx, channel) =
                        output(i) + T * (iDiff * mk(i) + T * ((3.f * (oDiff)-iDiff * (2.f * mk(i) + mk(i + 1))) + T * (-2.f * oDiff + iDiff * (mk(i) + mk(i + 1)))));
                }
                // equivalent to for-loop but allocates memory:
                // const ArrayXf T = (xDesired.segment(index(i), idxDiff) - input(i)) / iDiff;
                // y.block(index(i), channel, idxDiff, 1) = output(i) + T * (iDiff * mk(i) + T * ((3.f * (oDiff)-iDiff * (2.f * mk(i) + mk(i + 1))) + T * (-2.f * oDiff + iDiff
                // * (mk(i) + mk(i + 1)))));

...
{
  public:
    NoiseEstimationActivityDetection(Coefficients c = Coefficients()) : BaseAlgorithm{c}, activityDetection(convertToActivityDetectionCoefficients(c))
    {
  ...
  private:
    ...
    size_t getDynamicSizeVariables() const final { return activity.getDynamicMemorySize(); }
...
{
  public:
    NoiseEstimationOutputActivityDetection(Coefficients c = Coefficients()) : BaseAlgorithm{c}, activityDetection(convertToActivityDetectionCoefficients(c)) {}
...
// noise estimation based on an activity detector. The more activity in the power spectra, the more the output is smoothed. This version has an activity detector output as a
...
{
  public:
    NoiseEstimationOutputFusedActivityDetection(Coefficients c = Coefficients()) : BaseAlgorithm{c}, activityDetection(convertToActivityDetectionCoefficients(c)) {}

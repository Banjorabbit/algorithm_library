...
class MelScaleSpectrogram : public AlgorithmImplementation<MelScaleConfiguration, MelScaleSpectrogram>
{
  public:
    MelScaleSpectrogram(Coefficients c = Coefficients()) : BaseAlgorithm{c}
    {
        ...
        Eigen::ArrayXi cornerBins = ((c.nBands - 1) * 2 / c.sampleRate * freqs)
                                        .round()
                                        .cast<int>(); // corner bins, including 0 and nBands. Use round() instead of floor() to ensure increasing mel bin sizes
